...
class UserService {
  ...
  // Ban/Unban user
  async toggleUserStatus(id: string, isActive: boolean): Promise<SingleUserResponse> {
    return this.updateUser(id, { isActive });
  }

  // Cập nhật role user
  async updateUserRole(id: string, role: string): Promise<SingleUserResponse> {
    return this.updateUser(id, { roles: role });
  }

  // Xóa user (soft delete)
  async deleteUser(id: string): Promise<{ success: boolean; error?: string; message?: string }> {
    try {
      const url = this.getAbsoluteUrl(`/api/users/${id}`);
      ...
      if (!response.ok) {
        return {
          success: false,
          error: result.error || `Failed to delete user: ${response.statusText}`,
        };
      }
