...
export default function ContactUsPage() {
    ...
    const [selectedMethod, setSelectedMethod] = useState<'phone' | 'email' | 'zalo'>('email');
        ...
        <main>
            <div className="px-4 md:px-8 lg:px-20 py-10 bg-gray-50 min-h-screen">
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    {/* Dynamic Form */}
                    <div className={`bg-white rounded-lg shadow-md p-8 ${currentMethod?.formBg} border-l-4 ${selectedMethod === 'phone' ? 'border-green-400' :
                            selectedMethod === 'email' ? 'border-red-400' : 'border-blue-400'
                        }`}>
                        {/* Form Header */}
                        <div className="text-center mb-8">
                            ...
                            <div className="flex justify-center mt-2">
                                <div className={`w-24 h-1 rounded-full ${selectedMethod === 'phone' ? 'bg-green-400' :
                                        selectedMethod === 'email' ? 'bg-red-400' : 'bg-blue-400'
                                    }`}></div>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="email" className="text-gray-700">
                                    {selectedMethod === 'phone' ? 'Số điện thoại' :
                                        selectedMethod === 'zalo' ? 'Số điện thoại' : 'Email'} <span className="text-red-500">*</span>
                                </Label>
                                <Input
                                    id="email"
                                    name="email"
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="message" className="text-gray-700">
                                    {selectedMethod === 'zalo' ? 'Tin nhắn Zalo' : 'Tin nhắn'} <span className="text-red-500">*</span>
                                </Label>
                                <textarea
                                    id="message"
                                    name="message"
                                    ...
                                    onChange={handleInputChange}
                                    rows={6}
                                    className={`w-full min-h-[120px] px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 resize-vertical transition-all duration-300 ${selectedMethod === 'phone' ? 'focus:ring-green-400 focus:border-green-400' :
                                            selectedMethod === 'email' ? 'focus:ring-red-400 focus:border-red-400' :
                            ...
                            <div className="flex justify-end pt-4">
                                <Button
                                    ...
                                    className={`px-8 py-2 font-medium text-white transition-all duration-300 transform hover:scale-105 ${selectedMethod === 'phone' ? 'bg-green-600 hover:bg-green-700' :
                                            selectedMethod === 'email' ? 'bg-red-600 hover:bg-red-700' :
