...
export default function AddressSelectionModal({
    ...
    const [addressToEdit, setAddressToEdit] = useState<AddressType | undefined>(undefined);
                ...
                return (
                    <div className="pt-8">
                        <DialogHeader>
                         ...
                         <div className="py-4">
                            <AddressForm initialData={addressToEdit} onSuccess={handleFormSuccess} />
                        </div>
                    </div>
                );
            case 'LIST':
                    ...
                    <>
                        ...
                        <div className="mt-4 space-y-3 max-h-[60vh] overflow-y-auto p-1">
                            {addresses.map(addr => (
                                <div
                                    key={addr._id}
                                    className={`p-4 border rounded-lg cursor-pointer transition-all ${currentAddressId === addr._id ? 'border-blue-500 ring-2 ring-blue-500' : 'hover:border-gray-400'}`}
                                    onClick={() => handleSelectAndClose(addr)}
                                >
                                    <div className="flex justify-between items-start">
                                        <div className="font-semibold">{addr.fullName} {addr.isDefault && <span className="text-xs text-blue-600 ml-2">(Mặc định)</span>}</div>
                                        <Button variant="ghost" size="sm" className="h-auto p-1 text-blue-600 hover:bg-blue-50" onClick={(e) => { e.stopPropagation(); handleEditClick(addr); }}>
                            ...
                            {addresses.length === 0 && !isLoading && (
                                <p className="text-center text-muted-foreground py-8">Bạn chưa có địa chỉ nào.</p>
                            )}
                        </div>
                        <Button className="w-full mt-6" variant="outline" onClick={() => setView('CREATE')}>
                            Thêm địa chỉ mới
                        </Button>
                    </>
                );
