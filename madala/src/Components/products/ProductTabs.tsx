...
const ProductTabs: React.FC<ProductTabsProps> = ({
  ...
  // Memoize interactive star handlers
  ...
  const renderInteractiveStars = useCallback((currentRating: number, onRate?: (rating: number) => void) => {
    return Array.from({ length: 5 }, (_, i) => {
      const starValue = i + 1;
      ...
      return (
        <Star
          key={i}
          className={`w-6 h-6 cursor-pointer transition-colors ${isFilled ? 'fill-yellow-400 text-yellow-400' : 'text-gray-300 hover:text-yellow-300'
            }`}
  ...
  }, [hoverRating, handleStarHover, handleStarLeave]);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (userId) {
          try {
            const userRatingData = await RatingService.getUserRating(productId, userId);
  ...
  const totalReviews = safeRating.details ? Object.values(safeRating.details).reduce((sum, count) => sum + count, 0) : safeRating.count;
                  ...
                  {comments.map((commentItem, index) => (
                    <div key={commentItem._id || index} className="border-b pb-4">
                      <div className="flex items-center space-x-2 mb-2">
                        <span className="font-medium">{commentItem.userName}</span>
