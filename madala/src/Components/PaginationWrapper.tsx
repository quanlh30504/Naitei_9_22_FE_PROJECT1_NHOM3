        ...
        <div className={`${isCompact ? "mt-0" : "mt-8"} ${className} flex justify-center`}>
            <div className="pagination-container">
                <Pagination className="w-auto">
                    <PaginationContent className="flex items-center justify-center gap-0.5 sm:gap-1 bg-gradient-to-r from-white to-gray-50 dark:from-gray-800 dark:to-gray-700 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-1 sm:p-1.5 hover:shadow-xl transition-all duration-300 min-w-fit max-w-full h-10 sm:h-11">
                        ...
                        {visiblePages.map((page, index) => (
                            <PaginationItem key={`${page}-${index}`} className="flex-shrink-0">
                                {page === '...' ? (
                                    <PaginationEllipsis className="px-1 sm:px-2 py-2 text-xs sm:text-sm text-gray-500 dark:text-gray-400 border-0 min-w-[24px] sm:min-w-[32px] h-8 sm:h-9 flex items-center justify-center">
