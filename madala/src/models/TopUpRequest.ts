...
const TopUpRequestSchema: Schema = new Schema({
  ...
  payment: {
    type: new Schema<IPaymentDetails>({
      method: { type: String, enum: ['VIETQR', 'GATEWAY', 'MANUAL'], required: true },
  ...
  promoCodeId: { type: Schema.Types.ObjectId, ref: 'PromoCode', index: true, sparse: true },
...
TopUpRequestSchema.pre<ITopUpRequest>('save', function(next) {
  if (this.isModified('amount')) {
    if (this.amount.base + this.amount.bonus !== this.amount.total) {
      const err = new Error('Total amount must be the sum of base and bonus amounts.');
      return next(err);
    }
  }
  next();
...
TopUpRequestSchema.index(
  { expiresAt: 1 },
  { expireAfterSeconds: 0, partialFilterExpression: { status: { $in: ['PENDING', 'EXPIRED'] } } }
);

const TopUpRequest = mongoose.models.TopUpRequest || mongoose.model<ITopUpRequest>('TopUpRequest', TopUpRequestSchema);
